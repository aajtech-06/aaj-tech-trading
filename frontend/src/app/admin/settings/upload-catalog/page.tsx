...
export default function UploadCatalogPage() {
  ...
  const fetchCatalogs = async () => {
    try {
      ...
      const response = await fetch('https://aajtechtrading.in/api/catalog/');
      if (response.ok) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch('https://aajtechtrading.in/api/catalog/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleDeleteCatalog = async (filename: string) => {
    if (!confirm(`Are you sure you want to delete "${filename}"? This will permanently remove the catalog.`)) return;
    
    try {
      const response = await fetch(`https://aajtechtrading.in/api/catalog/${filename}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        fetchCatalogs();
      } else {
        alert('Failed to delete catalog from Cloudinary.');
      }
    } catch (error) {
      console.error(error);
      alert('Network error.');
  ...
  const handleRenameCatalog = async (oldFilename: string) => {
    ...
    try {
      const response = await fetch(
        `https://aajtechtrading.in/api/catalog/${oldFilename}/rename?new_name=${encodeURIComponent(newFilename)}`,
        {
          method: 'PUT',
        }
      );
      if (response.ok) {
        ...
        alert('Failed to rename catalog.');
      }
    } catch (error) {
      console.error(error);
      alert('Network error.');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12">
      ...
      {/* Catalog List / Management Section */}
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-black text-brand-dark mb-1">Manage Existing Catalogs</h2>
          <p className="text-gray-400 font-bold text-sm">Rename or delete catalog documents published on Cloudinary.</p>
        </div>
        ...
        {loadingCatalogs ? (
          ...
          <div className="bg-white rounded-[32px] border border-gray-100 p-12 text-center">
            <p className="text-gray-400 font-bold">No catalogs found. Upload a PDF file above to publish one.</p>
          </div>
        ) : (
