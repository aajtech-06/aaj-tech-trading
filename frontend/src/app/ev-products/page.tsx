...
const EVProductsContent = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch('https://aajtechtrading.in/api/ev/');
        if (res.ok) {
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          <motion.div {...fadeInUp} className="mb-12 md:mb-20">
            ...
            <div className="space-y-6 text-gray-600 dark:text-gray-300 text-base sm:text-xl leading-relaxed font-medium max-w-5xl">
              ...
              <p>
                From high-voltage battery interconnection harnesses to reliable charging station cables, our EV range is engineered to meet strict international automotive standards.
              </p>
              <p className="text-brand-dark dark:text-white font-bold pt-4">
                Explore our key categories of Electric Vehicle solutions:
              </p>
            </div>
          </motion.div>
                ...
                <div key={product.id} id={product.id} className={`flex flex-col ${index % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-8 md:gap-16 items-center scroll-mt-24`}>
                  ...
                  >
                    <div className="relative aspect-square w-full rounded-[2.5rem] overflow-hidden bg-[#f4f4f4] dark:bg-neutral-900 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-gray-100 dark:border-neutral-800 group cursor-pointer">
                      <Image
                        src={product.image?.startsWith('http') ? product.image : (product.image?.startsWith('/uploads/') ? `https://aajtechtrading.in${product.image}` : (product.image || "https://images.unsplash.com/photo-1563720223185-11003d516935?q=80&w=2070&auto=format&fit=crop"))}
                        alt={product.title}
                        fill
                        unoptimized
